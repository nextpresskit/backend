...
func (r *GormRepository) Create(ctx context.Context, post *postDomain.Post) error {
	...
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := upsertSEO(ctx, tx, postDomain.PostID(m.ID), post.SEO); err != nil {
			return err
		}
		if err := upsertMetrics(ctx, tx, postDomain.PostID(m.ID), post.Metrics); err != nil {
			return err
		}
		loaded, err := r.findByIDWithExtras(ctx, tx, postDomain.PostID(m.ID))
		if err != nil {
			return err
		}
		*post = *loaded
...
func (r *GormRepository) FindBySlug(ctx context.Context, slug string) (*postDomain.Post, error) {
	...
	return r.findByIDWithExtras(ctx, r.db.WithContext(ctx), postDomain.PostID(row.ID))
}
...
func (r *GormRepository) SetPrimaryCategory(ctx context.Context, postID postDomain.PostID, categoryID *string) error {
	return r.db.WithContext(ctx).
		Model(&gormPost{}).
...
func (r *GormRepository) findByIDWithExtras(ctx context.Context, db *gorm.DB, id postDomain.PostID) (*postDomain.Post, error) {
	var row gormPost
	if err := db.
		Where("id = ?", string(id)).
		First(&row).Error; err != nil {
	...
	p.Editors = make([]postDomain.UserSummary, 0, len(editorIDs))
...
func loadPostCoauthors(ctx context.Context, db *gorm.DB, postID string) ([]postDomain.UserSummary, error) {
	var rows []struct {
	...
	out := make([]postDomain.UserSummary, 0, len(rows))
	for _, r := range rows {
...
func loadUserSummary(ctx context.Context, db *gorm.DB, userID string) (*postDomain.UserSummary, error) {
...
func loadPostCategories(ctx context.Context, db *gorm.DB, postID string, primaryCategoryID *string) ([]postDomain.PostCategory, error) {
	...
	out := make([]postDomain.PostCategory, 0, len(rows))
	for _, r := range rows {
		isPrimary := primaryCategoryID != nil && *primaryCategoryID == r.ID
		out = append(out, postDomain.PostCategory{ID: r.ID, Name: r.Name, Slug: r.Slug, IsPrimary: isPrimary})
	}
	return out, nil
}
...
func loadPostTags(ctx context.Context, db *gorm.DB, postID string) ([]postDomain.PostTag, error) {
	...
	out := make([]postDomain.PostTag, 0, len(rows))
	for _, r := range rows {
		out = append(out, postDomain.PostTag{ID: r.ID, Name: r.Name, Slug: r.Slug})
	}
	return out, nil
}
...
func loadPostSeries(ctx context.Context, db *gorm.DB, postID string) (*postDomain.PostSeries, error) {
...
func loadPostSyndication(ctx context.Context, db *gorm.DB, postID string) ([]postDomain.PostSyndication, error) {
	...
	out := make([]postDomain.PostSyndication, 0, len(rows))
	for _, r := range rows {
		out = append(out, postDomain.PostSyndication{ID: r.ID, Platform: r.Platform, URL: r.URL, Status: r.Status})
	}
	return out, nil
}
...
func loadPostChangelog(ctx context.Context, db *gorm.DB, postID string) ([]postDomain.PostChangelogEntry, error) {
	...
	out := make([]postDomain.PostChangelogEntry, 0, len(rows))
	for _, r := range rows {
		var u *postDomain.UserSummary
		if r.UserID != nil {
			u, _ = loadUserSummary(ctx, db, *r.UserID)
		}
		out = append(out, postDomain.PostChangelogEntry{ID: r.ID, At: r.At, User: u, Note: r.Note})
	}
	return out, nil
}
...
func loadPostGallery(ctx context.Context, db *gorm.DB, postID string) ([]postDomain.PostGalleryItem, error) {
	...
	out := make([]postDomain.PostGalleryItem, 0, len(rows))
	for _, r := range rows {
		url := r.URL
		out = append(out, postDomain.PostGalleryItem{ID: r.ID, MediaID: r.MediaID, URL: &url, SortOrder: r.SortOrder, Caption: r.Caption, Alt: r.Alt})
	}
	return out, nil
}
...
func loadPostTranslations(ctx context.Context, db *gorm.DB, postID string) (*postDomain.PostTranslations, error) {
	...
	if err := db.WithContext(ctx).
		...
		if err == gorm.ErrRecordNotFound {
			return &postDomain.PostTranslations{GroupID: nil, Translations: []postDomain.PostTranslationEntry{}}, nil
		}
		return nil, err
	}
	gid := group.GroupID
	...
	out := make([]postDomain.PostTranslationEntry, 0, len(rows))
	for _, r := range rows {
		out = append(out, postDomain.PostTranslationEntry{PostID: r.PostID, Locale: r.Locale, Slug: r.Slug})
	}
	return &postDomain.PostTranslations{GroupID: &gid, Translations: out}, nil
}

func loadSEO(ctx context.Context, db *gorm.DB, postID postDomain.PostID) (*postDomain.PostSEO, error) {
	var row gormPostSEO
...
func loadMetrics(ctx context.Context, db *gorm.DB, postID postDomain.PostID) (*postDomain.PostMetrics, error) {
...
func upsertSEO(ctx context.Context, tx *gorm.DB, postID postDomain.PostID, seo *postDomain.PostSEO) error {
	if seo == nil {
		return nil
	}
	row := &gormPostSEO{
...
func upsertMetrics(ctx context.Context, tx *gorm.DB, postID postDomain.PostID, m *postDomain.PostMetrics) error {
	if m == nil {
		return nil
	}
	row := &gormPostMetrics{
...
func (r *GormRepository) DeleteSEO(ctx context.Context, postID postDomain.PostID) error {
	return r.db.WithContext(ctx).Where("post_id = ?", string(postID)).Delete(&gormPostSEO{}).Error
}

func (r *GormRepository) UpsertSEOOnly(ctx context.Context, postID postDomain.PostID, seo *postDomain.PostSEO) error {
	if seo == nil {
...
func (r *GormRepository) GetMetrics(ctx context.Context, postID postDomain.PostID) (*postDomain.PostMetrics, error) {
	return loadMetrics(ctx, r.db.WithContext(ctx), postID)
}

func (r *GormRepository) SetFeaturedImage(ctx context.Context, postID postDomain.PostID, mediaID *string, alt *string, width *int, height *int, focalX *float32, focalY *float32, credit *string, license *string) error {
...
func (r *GormRepository) SetPostSeries(ctx context.Context, postID postDomain.PostID, seriesID *string, partIndex *int, partLabel *string) error {
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
...
func (r *GormRepository) ReplaceCoauthors(ctx context.Context, postID postDomain.PostID, userIDs []string) error {
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
...
func (r *GormRepository) CreateGalleryItem(ctx context.Context, postID postDomain.PostID, mediaID string, sortOrder int, caption *string, alt *string) (string, error) {
...
func (r *GormRepository) UpdateGalleryItem(ctx context.Context, postID postDomain.PostID, itemID string, sortOrder *int, caption *string, alt *string) error {
	updates := map[string]any{}
	if sortOrder != nil {
...
func (r *GormRepository) DeleteGalleryItem(ctx context.Context, postID postDomain.PostID, itemID string) error {
	return r.db.WithContext(ctx).
		Where("id = ? AND post_id = ?", strings.TrimSpace(itemID), string(postID)).
...
func (r *GormRepository) CreateChangelog(ctx context.Context, postID postDomain.PostID, userID *string, note string) (string, error) {
...
func (r *GormRepository) DeleteChangelog(ctx context.Context, postID postDomain.PostID, changelogID string) error {
...
func (r *GormRepository) CreateSyndication(ctx context.Context, postID postDomain.PostID, platform, url, status string) (string, error) {
...
func (r *GormRepository) UpdateSyndication(ctx context.Context, postID postDomain.PostID, id string, platform, url, status *string) error {
...
func (r *GormRepository) DeleteSyndication(ctx context.Context, postID postDomain.PostID, id string) error {
	res := r.db.WithContext(ctx).
		Where("id = ? AND post_id = ?", strings.TrimSpace(id), string(postID)).
...
func (r *GormRepository) PutPostTranslation(ctx context.Context, postID postDomain.PostID, groupID *string, locale string) (string, error) {
...
func (r *GormRepository) ClearPostTranslation(ctx context.Context, postID postDomain.PostID) error {
...
func (r *GormRepository) ListSeries(ctx context.Context) ([]postDomain.Series, error) {
	var rows []gormSeriesRow
	if err := r.db.WithContext(ctx).Order("created_at DESC").Find(&rows).Error; err != nil {
		return nil, err
	}
	out := make([]postDomain.Series, 0, len(rows))
	for _, row := range rows {
		out = append(out, postDomain.Series{ID: row.ID, Title: row.Title, Slug: row.Slug, CreatedAt: row.CreatedAt, UpdatedAt: row.UpdatedAt})
	}
	return out, nil
}

func (r *GormRepository) CreateSeries(ctx context.Context, s *postDomain.Series) error {
...
func (r *GormRepository) FindSeriesByID(ctx context.Context, id string) (*postDomain.Series, error) {
	...
	return &postDomain.Series{ID: row.ID, Title: row.Title, Slug: row.Slug, CreatedAt: row.CreatedAt, UpdatedAt: row.UpdatedAt}, nil
}

func (r *GormRepository) UpdateSeries(ctx context.Context, s *postDomain.Series) error {
