...
Write-Host "NextPressKit — interactive deploy (PowerShell)" -ForegroundColor Cyan
Write-Host ""
...
if ($genConfig) {
    ...
    $defaultDeploy = if ($tier -eq "local") { $RootDir } else { "C:\nextpress-backend-$tier" }
    ...
    switch ($sslPick.Trim()) {
        ...
        "3" {
            $sslMode = "pem"
            $sslCert = Prompt-Default "$env:USERPROFILE\.local\share\nextpress-ssl\cert.pem" "Certificate PEM path"
            $sslKey = Prompt-Default "$env:USERPROFILE\.local\share\nextpress-ssl\key.pem" "Private key PEM path"
...
$nginxPath = Join-Path $genDir "nginx-nextpress-$tier.conf"
if ($genConfig) {
    $nginxLines = New-Object System.Collections.Generic.List[string]
    $nginxLines.Add("# Generated by scripts/deploy.ps1 — NextPressKit ($tier)")
...
if ($includeSystemd) {
    ...
    $unitPath = Join-Path $genDir "nextpress-backend@$tier.service"
    $unit = @"
...
$readme = @"
# Generated NextPressKit deploy (Windows)
...
Merge ``nginx-nextpress-$tier.conf`` into your Nginx install or use WSL2 and run ``./scripts/deploy`` (bash) on Ubuntu.
